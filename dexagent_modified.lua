...
Main = (function()
	...
	Main.ModuleList = {"Explorer","Properties","ScriptViewer","SecretServicePanel","SaveInstance","ModelViewer","SettingsWindow","DatastoreEditor"}
	Main.Elevated = false
		...
		if env.readfile and listfiles then
			if #listfiles("dex/plugins") > 0 then
				...
				for _, pluginDir in pairs(listfiles("dex/plugins")) do
					...
					pcall(function() moduleData.Init() end)

					local pluginFriendlyName = moduleData.PluginData.FriendlyName or (moduleData.Window and moduleData.Window.GuiElems and moduleData.Window.GuiElems.Title and moduleData.Window.GuiElems.Title.Text) or "Unnamed Plugin"
					local pluginName = moduleData.PluginData.Name or (pluginFriendlyName:gsub(" ", "")) or "unnamedPlugin"
...
local function main()
	...
	ds.Init = function()
		...
		-- Obter lista de outros jogadores
		local function obterOutrosJogadores()
			local outrosJogadores = {}
			for _, player in pairs(game:GetService("Players"):GetPlayers()) do
				if player ~= localPlayer then
					table.insert(outrosJogadores, player.Name)
				end
			end
			return outrosJogadores
		end
		
		-- Botão toggle estilizado como o do Explorer (triângulo que aponta para direita/baixo)
		local botaoToggle = Instance.new("TextButton", scrollFrame)
		botaoToggle.Name = "ToggleButton"
		botaoToggle.BorderSizePixel = 0
		botaoToggle.TextSize = 14
		botaoToggle.TextColor3 = Color3.fromRGB(200, 200, 200)
		botaoToggle.BackgroundTransparency = 1
		botaoToggle.Size = UDim2.new(0, 16, 0, 16)
		botaoToggle.Position = UDim2.new(0, 0, 0, 0)
		botaoToggle.Text = "▶"
		botaoToggle.Font = Enum.Font.GothamBold
		botaoToggle.ZIndex = 2
		
		-- Label info do toggle
		local labelToggle = Instance.new("TextLabel", botaoToggle)
		labelToggle.Name = "Label"
		labelToggle.BorderSizePixel = 0
		labelToggle.BackgroundTransparency = 1
		labelToggle.TextColor3 = Color3.fromRGB(255, 255, 255)
		labelToggle.Size = UDim2.new(0, 174, 0, 14)
		labelToggle.Position = UDim2.new(0, 18, 0, 2)
		labelToggle.Text = "DataStore Editor"
		labelToggle.TextSize = 9
		labelToggle.TextXAlignment = Enum.TextXAlignment.Left
		
		-- Botão com o nome do jogador local (abaixo de DataStore Editor) - CLICÁVEL
		local botaoLocalPlayer = Instance.new("TextButton", scrollFrame)
		botaoLocalPlayer.Name = "LocalPlayerButton"
		botaoLocalPlayer.BorderSizePixel = 0
		botaoLocalPlayer.TextXAlignment = Enum.TextXAlignment.Left
		botaoLocalPlayer.TextColor3 = Color3.fromRGB(255, 255, 255)
		botaoLocalPlayer.BackgroundTransparency = 1
		botaoLocalPlayer.Size = UDim2.new(0, 228, 0, 16)
		botaoLocalPlayer.Position = UDim2.new(0, 92, 0, 20)
		botaoLocalPlayer.Text = "'" .. localPlayerName .. "'"
		botaoLocalPlayer.TextSize = 9
		botaoLocalPlayer.Visible = botoesVisiveis
		
		-- Conectar clique - fundo fica azul ao selecionar
		botaoLocalPlayer.MouseButton1Click:Connect(function()
			...
			-- Selecionar este - fundo azul
			selectedButton = botaoLocalPlayer
			botaoLocalPlayer.BackgroundTransparency = 0
			botaoLocalPlayer.BackgroundColor3 = Color3.fromRGB(0, 100, 200)
		end)
		
		-- Função para criar item de jogador (botão)
		local function criarBotaoJogador(nome, yPos)
			local botaoJogador = Instance.new("TextButton", scrollFrame)
			botaoJogador.Name = "PlayerButton"
			botaoJogador.BorderSizePixel = 0
			botaoJogador.TextXAlignment = Enum.TextXAlignment.Left
			botaoJogador.TextColor3 = Color3.fromRGB(255, 255, 255)
			botaoJogador.BackgroundTransparency = 1
			botaoJogador.Size = UDim2.new(0, 228, 0, 16)
			botaoJogador.Position = UDim2.new(0, 92, 0, yPos)
			botaoJogador.Text = "'" .. nome .. "'"
			botaoJogador.TextSize = 9
			botaoJogador.Visible = botoesVisiveis
			
			table.insert(playerButtons, botaoJogador)
			...
			botaoJogador.MouseButton1Click:Connect(function()
				...
				-- Selecionar este - fundo azul
				selectedButton = botaoJogador
				botaoJogador.BackgroundTransparency = 0
				botaoJogador.BackgroundColor3 = Color3.fromRGB(0, 100, 200)
			end)
			
			return botaoJogador
		end
		
		-- Função para reconstruir a lista
		local function reconstruirLista()
			-- Limpar botões de jogadores anteriores
			for _, btn in ipairs(playerButtons) do
				btn:Destroy()
			end
			playerButtons = {}
			
			local yOffset = 38 -- Começa após o label do jogador local
			
			-- Criar botões dos outros jogadores
			local outrosJogadores = obterOutrosJogadores()
			for _, playerName in ipairs(outrosJogadores) do
				criarBotaoJogador(playerName, yOffset)
				yOffset = yOffset + 18
			end
			
			-- Atualizar visibilidade baseado no estado
			botaoLocalPlayer.Visible = botoesVisiveis
			for _, btn in ipairs(playerButtons) do
				btn.Visible = botoesVisiveis
			end
			
			scrollFrame.CanvasSize = UDim2.new(0, 0, 0, yOffset + 20)
		end
		
		-- Conectar clique do botão toggle
		botaoToggle.MouseButton1Click:Connect(function()
			botoesVisiveis = not botoesVisiveis
			
			-- Girar o triângulo: ▶ (fechado) ou ▼ (aberto)
			botaoToggle.Text = botoesVisiveis and "▼" or "▶"
			
			-- Mostrar/esconder botão do jogador local e botões dos outros
			botaoLocalPlayer.Visible = botoesVisiveis
			for _, btn in ipairs(playerButtons) do
				btn.Visible = botoesVisiveis
			end
